...
- **Image treatment ("taped snapshot"):** contained content photos are wrapped in `<figure class="snapshot">` — cream print border, `<figcaption>` in normal flow below the image, two tape strips as pseudo-elements over the top corners, hard ink offset shadow (`--color-shadow-ink`, the same ink the cards cast). Genuinely full-bleed atmosphere bands stay edge-to-edge and are not taped — a deliberate mixed treatment.
