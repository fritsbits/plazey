...
- Tailwind v4 is installed (`@tailwindcss/vite`, `@import "tailwindcss"` at the top of `global.css`), but only its spacing and layout utilities get used. All visual design lives in `global.css` under `@layer components`, driven by design tokens. The huisstijl has shipped; see Design Context below.
- **Gotcha: a utility beats your component rule whatever the specificity.** Cascade layers order `components` before `utilities`, so you cannot override something like `Container.astro`'s `max-w-4xl` from `@layer components`. Give the element its own wrapper class instead (see `.photo-wall-container`).
...
- `PhotoWall.astro` — the look-back gallery, home page, `aftermovie` phase only. Takes `lang`; the photo list and its per-language alt text live in `src/config/photos-2026.ts` so NL and FR share one ordering. Masonry via `columns`, and the prints are the **untaped** variant of `.snapshot` (see Image treatment below).
...
**What the `aftermovie` phase changes.** Home: thank-you H1, no date line, no CTA row, and `PhotoWall` between the hero and the green band with a text link out to Facebook under it. Nav: the `reveal` links, with Programma relabelled "Programma 2026" / "Programme 2026". Programma, Praktisch and Vrijwilliger each gain a `.season-banner` note. Praktisch and Vrijwilliger also swap their H1 / signup copy to past-tense and next-year framing. Everything else on those pages stays evergreen on purpose: it describes an edition that happened and mostly still holds for the next one, so flipping the phase back is the only thing needed to reopen the site.

**The "Vandaag" badge is date-driven, not phase-driven.** The programme page compares the browser clock against each day's `data-date`, so the badge and the scroll-to-today expire on their own at midnight. Never gate them on `SITE_PHASE` — the site is a static build that only redeploys on a CMS save, so the phase can lag reality by days.
...
**Programme filter (Programma pages):** one axis only — **type**. A row of single-select toggle chips (`<button aria-pressed>`); nothing pressed means "show everything", so there is no "all types" chip, and pressing the active chip clears it. Filtering show/hides rendered HTML, no re-fetch; `?type=` syncs to the URL for shareable links; an `aria-live` region announces the count (silent on first paint). The chip bar is JS-only, so it ships `hidden` and the script reveals it.

**One chip covers several types.** Nine types would mean nine chips, six of which filter the programme down to two items or fewer — a table of contents, not a filter. `typeGroups` in `src/config/programme-labels.ts` maps them onto four buckets (Muziek / Kinderen en families / Workshop / Te zien), each chip carrying `data-type` (group key) and `data-types` (the types it stands for); the script matches by membership. **A new type must be added to a group there, or no chip will ever reach it.** Cards keep their own precise `typeLabels` badge, so a Fête Foraine card still reads "Kermis" under the Kinderen en families chip. A group renders only when one of its types has items, so an empty result stays unreachable — no empty state exists. `?type=` takes a group key; an unknown value resolves to the group containing it, so links shared before the grouping (`?type=expo`) still land on the right chip. The `kids` group deliberately keeps that name because the Kinderen section on Praktisch links `/nl/programma/?type=kids` (FR `/fr/programme/?type=kids`) — that cross-page link is the only entry point into the kids filter, so it must keep resolving.
...
- **Radius:** a 6-step scale, not one project-wide value — `--radius-lg` 24px (images), `--radius-md` 20px (cards), `--radius-sm` 14px (buttons), `--radius-xs` 12px (tags/marks), `--radius-xxs` 3px (photo print corners), `--radius-pill` 999px (chips/pill buttons).
- **Image treatment ("taped snapshot"):** contained content photos are wrapped in `<figure class="snapshot">` — a cream print border, a `<figcaption>` in normal flow below the image, two tape strips drawn as pseudo-elements over the top corners, and a hard ink offset shadow (`--color-shadow-ink`, the same ink the cards cast). Tilts alternate `.snapshot--tilt-l` / `--tilt-r` down a page and collapse under `prefers-reduced-motion`; the tape rotation stays, because it is shape rather than motion. **A snapshot inside an already-tilted `.paper-sheet--tilt-*` section takes no tilt modifier** — two rotations compound. `.snapshot--solo` caps a standalone print at 32rem so the fixed 88px tape stays in proportion. Inside a `.split-section`, `.split-image` is a plain `<div>` wrapper with the `<figure class="snapshot">` nested inside, because `.split-image`'s own 2rem padding would otherwise swallow the print border. Never tape two photos side by side — their tape corners collide. Genuinely full-bleed atmosphere bands stay edge-to-edge and are not taped: a deliberate mixed treatment. **A gallery is the third case:** `.photo-wall` keeps the print border, the ink offset and the tilts but drops the tape, because tape overhangs a print by roughly 15px on each side and neighbours in a grid would knock corners. Tilts there run on a four-step cycle rather than an l/r alternation, so a three-column wall doesn't land the same lean in the same column position on every row. (The earlier `.sticker-img` accent-colour offset is deleted — photos must not cast a coloured shadow when every card casts translucent ink.)
- **Season note (`.season-banner`):** the post-festival note used on Programma, Praktisch and Vrijwilliger. Cream ground, not one of the named accents: it has to read on the pink home, the yellow Praktisch page, the orange Vrijwilliger page and the green Programma panel, and cream is the only ground that pops on all four. Carries the poster signature so it doesn't land as a plain box.
