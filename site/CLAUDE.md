...
**One chip covers several types.** Nine types would mean nine chips, six of which filter the programme down to two items or fewer — a table of contents, not a filter. `typeGroups` in `src/config/programme-labels.ts` maps them onto four buckets (Muziek / Kinderen / Workshop / Te zien), each chip carrying `data-type` (group key) and `data-types` (the types it stands for); the script matches by membership. **A new type must be added to a group there, or no chip will ever reach it.** Cards keep their own precise `typeLabels` badge, so a Fête Foraine card still reads "Kermis" under the Kinderen chip. A group renders only when one of its types has items, so an empty result stays unreachable — no empty state exists. `?type=` takes a group key; an unknown value resolves to the group containing it, so links shared before the grouping (`?type=expo`) still land on the right chip. The `kids` group deliberately keeps that name because the kids callout links `?type=kids`.
